...
// use circ::{AtomicRc, AtomicWeak, CompareExchangeError, EdgeTaker, OwnRc, Rc, RcObject, Snapshot};
...
//             (Key::Fin(_), Key::Inf1) => Some(Less),
//             (Key::Fin(_), Key::Inf2) => Some(Less),
//             (Key::Inf1, Key::Fin(_)) => Some(Greater),
//             (Key::Inf1, Key::Inf1) => Some(Equal),
//             (Key::Inf1, Key::Inf2) => Some(Less),
//             (Key::Inf2, Key::Fin(_)) => Some(Greater),
//             (Key::Inf2, Key::Inf1) => Some(Greater),
//             (Key::Inf2, Key::Inf2) => Some(Equal),
//         }
//     }
// }
...
//     fn pop_edges(&mut self, out: &mut EdgeTaker<'_>) {
//         match self {
//             Update::Insert { new_internal, .. } => out.take(new_internal),
//             Update::Delete { pupdate, .. } => out.take(pupdate),
//         }
//     }
// }
...
//     #[inline]
//     fn search(&mut self, key: &K, root: &AtomicRc<Node<K, V>>) {
//         root.load(&mut self.l, Ordering::Relaxed);
//         loop {
//             if self.l.as_ref().unwrap().is_leaf() {
//                 break;
//             }
...
//             let p_node = self.p.as_ref().unwrap();
//             p_node.update.load(&mut self.pupdate, Ordering::Acquire);
//             match p_node.key.cmp(key) {
//                 Greater => p_node.left.load(&mut self.l, Ordering::Acquire),
//                 _ => p_node.right.load(&mut self.l, Ordering::Acquire),
//             }
//         }
//     }
// }
...
//     pub fn find<'h>(&self, key: &K, cursor: &'h mut Cursor<K, V>) -> Option<&'h V> {
//         cursor.search(key, &self.root);
//         let l_node = cursor.l.as_ref().unwrap();
//         if l_node.key.eq(key) {
//             Some(l_node.value.as_ref().unwrap())
//         } else {
//             None
//         }
//     }

//     pub fn insert(&self, key: &K, value: V, cursor: &mut Cursor<K, V>) -> bool {
//         loop {
//             cursor.search(key, &self.root);
//             let l_node = cursor.l.as_ref().unwrap();
...
//                     }
//                     Err(e) => {
//                         let mut new_pupdate = e.desired;
//                         unsafe {
//                             let mut new_pupdate_failed = new_pupdate.deref_mut();
//                             if let Update::Insert { new_internal, .. } = &mut new_pupdate_failed {
//                                 let mut new_internal_failed = new_internal.take();
//                                 new_internal_failed.deref_mut().left.take().finalize(guard);
//                                 new_internal_failed.deref_mut().right.take().finalize(guard);
//                                 new_internal_failed.finalize(guard);
//                             }
//                             new_pupdate.finalize(guard);
//                         }
...
//     pub fn delete<'g>(&self, key: &'g K, guard: &'g Guard) -> Option<&'g V> {
//         loop {
...
//             let other = if p_ref.right.load(Ordering::Acquire, guard).ptr_eq(l) {
//                 &p_ref.left
//             } else {
//                 &p_ref.right
//             };
...
//                 let mut guard = cs();
//                 for i in keys {
//                     assert_eq!(i.to_string(), *map.delete(&i, &guard).unwrap());
//                     guard = cs();
//                 }
//             });
//         }
...
//                 let mut guard = cs();
//                 for i in keys {
//                     assert_eq!(i.to_string(), *map.find(&i, &guard).unwrap());
//                     guard = cs();
//                 }
//             });
//         }
